...
public interface OrdersGoodMapper extends BaseMapper<OrdersGood> {
    ...
    @Select("SELECT DATE_FORMAT(orders.date, '%Y年%m月%d日 %H:%i:%s') as time,\n" +
            "    order_id as orderId,\n" +
            "    IFNULL(consumer.`name`, '纯路人') as username,\n" +
            "    good.good_name as goods,\n" +
            "    orders_good.good_num as number,\n" +
            "    good.price,\n" +
            "    orders_good.good_num*good.price as total\n" +
            "    FROM orders\n" +
